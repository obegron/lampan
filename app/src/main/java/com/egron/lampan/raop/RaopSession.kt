...
class RaopSession(private val host: String, private val port: Int = 7000, private val logCallback: ((String) -> Unit)? = null) {
    ...
    suspend fun connect() {
        ...
        clientControlSocket = DatagramSocket(0) 
        clientTimingSocket = DatagramSocket(0)
        clientAudioSocket = DatagramSocket(0)
            ...
            try {
                while (!socket.isClosed) {
                    ...
                    if (name == "Timing" && len >= 32 && buffer[0] == 0x80.toByte() && buffer[1] == 0xD2.toByte()) {
                        ...
                        System.arraycopy(buffer, 24, response, 12, 8)
                        val nowNtp = ntpTime()
                        putLong(response, 20, nowNtp)
                        putLong(response, 28, nowNtp)
    ...
    fun sendFrame(pcm: ByteArray) {
        ...
        try {
            ...
            val dp = DatagramPacket(packetBytes, packetBytes.size, InetAddress.getByName(host), serverAudioPort)
